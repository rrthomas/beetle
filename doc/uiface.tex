...
It can be made separately as the Make target {\tt beetle}. Run {\tt beetle --help} to see its syntax; most importantly, if given a single parameter, the name of an object file, the file is loaded and run. Any further arguments will be passed to Beetle, accessible via the {\tt ARGC} and {\tt ARG} library calls.
...
\rule[-2mm]{0mm}{6mm}\bf Register & \bf Initial value \\ \midrule
{\tt EP} & 10h \\
{\tt MEMORY} & \emph{see above} \\
{\tt 'THROW} & 0h \\ \bottomrule
\end{tabular}
...
The command {\tt DEBUG} enables the display of debugging messages. This is likely to be helpful only for debugging C Beetle itself. It may be used in the two forms
...
where the first toggles the debugging state, and the second switches debugging off if its argument is zero, or on otherwise.

The command-line switch \verb|--debug| is equivalent to \verb|DEBUG 1|.
...
\begin{center}
\begin{tabular}{cc} \toprule
\rule[-2mm]{0mm}{6mm}\bf Command & \bf Minimum abbreviation \\ \midrule
{\tt >D} & {\tt >} \\
{\tt >R} & {\tt >R} \\
{\tt COUNTS} & {\tt C} \\
{\tt DISASSEMBLE} & {\tt D} \\
