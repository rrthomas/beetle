...
\usepackage{a4wide,newpxtext,booktabs,url}
...
\\}
\newcommand{\opcodetbltwo}[4]{#1h & {\tt #2} & #3h & {\tt #4} \\}
...
The bForth instruction set is listed in sections~\ref{stackinst} to~\ref{accessinst},
...
\instp{EP@}{bracket-create}{56h}{-- a-addr}{Push {\tt EP} on to the stack.}
\instp{S0@}{s-nought-fetch}{57h}{-- a-addr}{Push {\tt S0} on to the stack.}
\instp{\#S@}{hash-s-fetch}{58h}{-- u}{Push {\tt \#S} on to the stack.}
\instp{R0@}{r-nought-fetch}{59h}{-- a-addr}{Push {\tt R0} on to the stack.}
\instp{\#R@}{hash-r-fetch}{5Ah}{-- u}{Push {\tt \#R} on to the stack.}
\instp{'THROW@}{tick-throw-fetch}{5Bh}{-- a-addr}{Push {\tt 'THROW} on to the stack.}
\instp{'THROW!}{tick-throw-store}{5Ch}{a-addr --}{Set {\tt 'THROW} to \spic{a-addr}.}
\instp{MEMORY@}{memory-fetch}{5Dh}{-- a-addr}{Push {\tt MEMORY} on to the stack.}
\instp{'BAD@}{tick-bad-fetch}{5Eh}{-- a-addr}{Push {\tt 'BAD} on to the stack.}
\instp{-ADDRESS@}{not-address-fetch}{5Fh}{-- a-addr}{Push {\tt -ADDRESS} on to the stack.}
\end{description}
...
\instr{EXECUTE}{46h}{\aaddr1 --}{ -- \aaddr2}{Push {\tt EP} on to the return stack, put \spic{\aaddr1} into {\tt EP}, then perform the action of {\tt NEXT}.}
\instpr{@EXECUTE}{fetch-execute}{47h}{\aaddr1 --}{ -- \aaddr2}{Push {\tt EP} on to the return stack, put the contents of \spic{\aaddr1} into {\tt EP}, then perform the action of {\tt NEXT}.}
\end{description}
...
\begin{description}
\inst{LIB}{60h}{i*x n -- j*x}{Call library routine \spic{n}. The parameters passed and returned depend on \spic{n}. If the library routine is not currently available, raise exception $-257$.}
\inst{LINK}{61h}{i*x --}{Make a subroutine call to the routine at the address given (in the host machine's format, padded out to a number of cells) on the data stack. The size and format of this address are machine-dependent.}
\end{description}
...
% FIXME: add S0@, #S, R0@, #R, 'THROW@, 'THROW!, MEMORY@, 'BAD@, -ADDRESS@ once we’ve reordered the opcodes
\end{tabular}
