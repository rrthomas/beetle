...
\begin{description}
\instp{THROW}{throw}{--}{Put the contents of {\tt EP} into {\tt 'BAD}, then load {\tt EP} from {\tt 'THROW}. Perform the action of {\tt NEXT}. If {\tt 'THROW} contains an out of range or unaligned address stop Beetle, returning reason code $-258$ to the calling program (see section~\ref{calls}).}
\inst{HALT}{x --}{Stop Beetle, returning reason code \spic{x} to the calling program (see section~\ref{calls}). If {\tt SP} is out of range or unaligned, $-257$ is returned as the reason code.}
\end{description}
...
Instructions with opcodes from 80h to FEh inclusive may be implemented to provide system-dependent functionality, such as I/O.
...
undefined opcodes (61h--FEh) raise exception $-256$.
...
\opcodetbl{1F}{-}	{40}{RP@}	{80h–FE}{\textrm{extra instructions}}
