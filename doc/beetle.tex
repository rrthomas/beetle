...
\date{29th October 1993; revised 10th February 2018}
...
provides access to a standard library which mimics ANS Forth I/O words. Machine
...
{\tt M0} & ``m-zero'' & The address of Beetle's main memory on the host
...
address (for most operations, only addresses between zero and {\tt MEMORY $-$ 1}
inclusive are valid; for memory fetch and store instructions other addresses may
also be valid at the discretion of the implementation), either by an
...
{\tt EP@} pushes {\tt EP} on to the stack (this is not a part of normal flow
control!). {\tt NEXT} performs an instruction fetch when Beetle runs out of
...
\opcodetbl{18}{U>}	{37}{1LSHIFT}	{56}{EP@}
\opcodetbl{19}{0}	{38}{1RSHIFT}	{57}{LIB}
\opcodetbl{1A}{1}	{39}{@}		{58}{LINK}
\opcodetbl{1B}{-1}	{3A}{!}		{59}{RUN}
\opcodetbl{1C}{CELL}	{3B}{C@}	{5A}{STEP}
\opcodetbltwo{1D}{-CELL}	{3C}{C!}
\opcodetbl{1E}{+}	{3D}{+!}	{FF}{NEXT}	\bottomrule
\end{tabular}
...
\iface{load\_object}{file, address}{integer}{Load the object module
    ...
    the address is out of range, return $-1$. Otherwise load the bForth code into
