...
\begin{description}
\ifacec{int init\_beetle(BYTE *b\_array, long size)}{{\tt size} is
...
All the registers are initialised as per~\cite{beetle} ({\tt EP} is set to $16$), and those held in Beetle's memory as well are copied there.}
\end{description}
...
\ifacec{UCELL mem\_align(void)}{Like Forth’s {\tt ALIGN}, rounds up the address at which the next high memory allocation will occur to the next cell.}
\end{description}
